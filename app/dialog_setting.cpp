...
void Dialog_setting::on_pushButton_clicked()
{
    ...
    if(ui->linedialog_db->text() == "" || ui->linedialog_ip->text() == "" || ui->linedialog_pass->text() == "" || ui->linedialog_user->text() == ""){
        ...
        db.setHostName(ui->linedialog_ip->text());
        db.setPassword(ui->linedialog_pass->text());
        db.setUserName(ui->linedialog_user->text());
        db.setDatabaseName(ui->linedialog_db->text());
        if(db.open()){
            n.setText("Ket noi thanh cong! Ban co the luu lai");
            n.exec();
        }else{
            n.setText("Khong the ke noi den co so du lieu!\nVui long kiem tra lai thong tin ket noi."+db.lastError().text());
            n.exec();
        }
    }
}
